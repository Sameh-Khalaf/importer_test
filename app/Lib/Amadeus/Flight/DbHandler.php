...
class DbHandler
{
    ...
    public static function saveSegments(SegmentsCollection &$segmentsCollection, IdentCollection &$identCollection)
    {
        ...
        for ($i = 0; $i < $segmentsCounter; $i++) {
            if($segmentsCollection->getByIndex('dep_city',$i) == '') continue;
    ...
    public function saveInvoiceRemarks(IdentCollection &$identCollection, InvoiceRemarksCollection &$invoiceRemarksCollection)
    {
        if (!empty($invoiceRemarksCollection->toArray()[0]['remark_type'])) {
            ...
            for ($i = 0; $i < $invoiceRemarksCounter; $i++) {
                ...
                $invoiceRemarksModel->setConnection('pgsql_crs_'.self::$agent);
        ...
        if (isset($customRemarksCollection) && $customRemarksCollection->count()) {
            ...
            foreach ($customRemarksCollection->toArray() as $single) {
                ...
                if($identCollection->offsetGet('match_code') !== '') {
                    ...
                    if(isset($remarksSettings[0]) && !empty($remarksSettings)){
                        foreach ($remarksSettings[0] as $singleRemark){

                            //if equal then this remark related to traveller
                            if($singleRemark['code'] == $single['field_name']){

                                $participantsId = $participantsCollection->offsetGetByIndex('id', $single['participants_id']);
                                if (null !== $participantsId && !empty($participantsId)) {
                                    $single['participants_id'] = $participantsId;
                                    ...
                                    $customeRemarksModel->setConnection('pgsql_crs_'.self::$agent);
                                    try {
                                        if (static::$output) {
                                            ...
                                            if ($single['remark'] == 'RM*IDINV') {
                                                DB::connection("pgsql_crs_".self::$agent)->table('ident')->where('id',$single['pnr_id'])
                                                    ->update(['has_online_invoice' => true]);
                                                // Ident::where('id', $single['pnr_id'])->update(['has_online_invoice' => true]);
                                            }

                                            if ($single['remark'] == 'RM*IDVCHR') {
                                                DB::connection("pgsql_crs_".self::$agent)->table('ident')->where('id',$single['pnr_id'])
                                                    ->update(['has_online_voucher' => true]);
                                            }
                                        }
                                    } catch (\Exception $e) {
                                        throw new \Exception($e->getMessage().' '.__FILE__.' '.__LINE__);
                                        //var_dump($customeRemarksModel, $customRemarksCollection, $e->getMessage(), $participantsCollection, $participantsId, static::$file);
                                        die;
                                    }
                                }else{
                                    throw new \Exception('participants_id is null'.' '.__FILE__.' '.__LINE__);
//                                    var_dump('participants_id is null');
                                    die;
                                }
                                continue;
                            }
...
//                var_dump($fileChecksumModel);die;
