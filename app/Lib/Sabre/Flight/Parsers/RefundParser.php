        ...
        if (isset($fileDataArray['M5'])) {
            ...
            if(isset($fileDataArray['M2'])){
                ...
                foreach ($fileDataArray['M5'] as $singleLine) {
                    $refundIdentifier = trim(substr($singleLine,7,1));
                    if($refundIdentifier != 'R' ){
                        continue;
                    }
                    ...
                    if(in_array($participantsId,$this->validParticipants)) {
                        $price['fare_amount'] = Sab_matchNewFareAmount($singleLine);
                        $price['fare_currency'] = 'EGP';
                        $price['tax_amount'] = Sab_matchNewTaxAmount($singleLine);
