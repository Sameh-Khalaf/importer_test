...
//                $identCollection->put('total_pnr_passengers', matchTotalPassengerInPNR($singleLine));
        ...
        $matchCode = Sab_matchMatchCode($data[0]);
        $identCollection->put('match_code', $matchCode);
        $identCollection->put('sabre_inv_number', trim(substr($data[0],36,7)));

        //Prices MX

        if(isset($fileDataArray['M5'])){

            if(isPricesInMx($data[0]) && 1 != 1) {
            ...
            foreach ($fileDataArray['M5'] as $key=>$single) {
                ...
                if(strpos($single,'/EMD') !== false){
                    continue;
                }
                if (preg_match('/F|D\/E$/', $single) == false) {
                    continue;
                }
                $identifier = trim(substr($single,7,1));
...
//PRINT_R($priceCollection->toArray());die;
...
//                $segmentsCollection->putByIndex('stop_over', Sab_matchStopOver($singleLine), $segmentCounter);
        ...
        if(isset($fileDataArray['M1'])){
            foreach ($fileDataArray['M1'] as $key=>$singleLine) {
                //check if this is valid participant in mx
                $participantsId = substr($singleLine,2,2);
                if(!in_array($participantsId,$this->validParticipants)) {
                    continue;
                }
...
//                $participantsCollection->putByIndex('price', $priceCollection->getByIndex('fare_amount',$participantsId), $this->participantCounter);
            }
        }
        ...
        if(isset($fileDataArray['M5'])){
            foreach ($fileDataArray['M5'] as $key=>$singleLine){
                ...
                if(strpos($singleLine,'/EMD') !== false){
                    continue;
                }
                if (preg_match('/F|D\/E$/', $singleLine) == false) {
                    continue;
                }
                $identifier = trim(substr($singleLine,7,1));
                if($identifier != '') continue;

                $identCollection->put('version', 1);

                if(in_array($participantsId,$this->assignedTicketsParticipants)){
                    //conjuctive ticket
                    $counter = ++$this->ticketCounter;
                    $this->ticketDataCollectionUpdateIndexArray[] = $counter;


                    $ticketDataCollection->putByIndex('number', trim(Sab_matchTicketNumber($singleLine)), $counter);
...
//                    $ticketDataCollection->putByIndex('name', $participantsCollection->offsetGetByIndex('name', $this->participantCounter), $counter);
                    $ticketDataCollection->putByIndex('name', $participantsCollection->offsetGetByIndexNumber('name', $participantsId), $counter);

                    $ticketDataCollection->putByIndex('fare_amount', $priceCollection->getByIndex('fare_amount',$ticketIdentifier.$participantsId), $counter);
                    $ticketDataCollection->putByIndex('fare_currency', 'EGP', $counter);
                    $ticketDataCollection->putByIndex('tax_amount',$priceCollection->getByIndex('tax_amount',$ticketIdentifier.$participantsId),$counter);
                    $ticketDataCollection->putByIndex('tax_currency','EGP',$counter);
                    $ticketDataCollection->putByIndex('equiv_amount', '0.00', $counter);
                    $ticketDataCollection->putByIndex('equiv_currency', '', $counter);


                    $ticketDataCollection->putByIndex('conjunctive_flag', 'false', $counter);
                    $ticketDataCollection->putByIndex('tour_operator', $identCollection->offsetGet('tktoffice_id'), $counter);
                    $ticketDataCollection->putByIndex('participants_index',$participantsId, $counter);

                    //$ticketDataCollection->putByIndex('commission_amount', Sab_matchCommissionAmount($singleLine), $counter);
                    $ticketDataCollection->putByIndex('fop', Sab_matchFormOfPayment($fileDataArray['M5'][0]), $counter);

                    //$ticketDataCollection->putByIndex('commission_amount', Sab_matchCommissionAmount($singleLine), $counter);
                    $ticketDataCollection->putByIndex('valid_carrier', Sab_matchValidatingCarrier($singleLine), $counter);


                    $ticketDataCollection->putByIndex('commission_rate', '0.00',  $counter);
                    $ticketDataCollection->putByIndex('commission_vat', '0.00',  $counter);
                    $ticketDataCollection->putByIndex('company_own_cc', 'false',  $counter);
                    $ticketDataCollection->putByIndex('partially_paid', 'false',  $counter);
                    ...
                    $ticketDataCollection->putByIndex('participants_index', $participantsId, $this->ticketCounter);
