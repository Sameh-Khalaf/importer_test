...
//        var_dump($this->items,$index,$key);die;
        throw new \Exception('Key {' . $key . '} does not exist in the ' . get_class($this) . ' ' . __FILE__);
    }
    public function offsetGetByIndexNumber($key,$index)
    {
        foreach ($this->items as $k=>$single)
        {
            if($key == "*" && $single['number'] == $index){
                return $single;
            }
            if(isset($single[$key]) && $single['number'] == $index){
                return $single[$key];
            }
        }
        throw new \Exception('Key {' . $key . '} does not exist in the ' . get_class($this) . ' ' . __FILE__);
