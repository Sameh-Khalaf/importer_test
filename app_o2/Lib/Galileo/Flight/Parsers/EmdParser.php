...
//                $identCollection->put('total_pnr_passengers', matchTotalPassengerInPNR($singleLine));
            ...
            if ($threeCharsTag == 'A02') {
                ...
                $fareLines = Gal_getIdentifierIndexFromArray($data,'A29'.$fareNumber)[0];

                Gal_setPriceCollectionEmd($priceCollection, $data[$fareLines], $fareNumber);
                ...
                $ticketNumber = $this->validAirlineNumber . '-' . Gal_matchTicketNumberEMD($data[$fareLines]);
                ...
                //check icw
                $fareLines = Gal_getIdentifierIndexFromArray($data,'ED-/RL-',0)[0];
                $originalNumber = Gal_matchTicketNumberOriginalEMD($data[$fareLines]);
...
//                $segmentsCollection->putByIndex('carrier', matchSegmentsCarrier($singleLine), $segmentCounter);
                ...
                if($priceCollection->getByIndex('fare_currency',0) !== 'EGP'){
                    $segmentsCollection->putByIndex('fare_currency', 'EGP', $segmentCounter);
                    $segmentsCollection->putByIndex('fare', $priceCollection->getByIndex('equiv_amount',0), $segmentCounter);
                }else {
                    $segmentsCollection->putByIndex('fare_currency', 'EGP', $segmentCounter);
                    $segmentsCollection->putByIndex('fare', $priceCollection->getByIndex('fare_amount',0), $segmentCounter);
                }
                $segmentsCollection->putByIndex('total_tax', $priceCollection->getByIndex('tax_amount',0), $segmentCounter);
