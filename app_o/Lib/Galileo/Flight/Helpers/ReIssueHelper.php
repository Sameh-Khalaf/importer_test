...
//                number like '%$ticketNumber%' and ticket_type = '$ticketType' and pnr_id in (select id from ident where  agent = '$agent' and crs_id = '1'))";
        $sql ="SELECT EXISTS (select pnr_id from ticketdata where 
                number like '%$ticketNumber%' and original_number like '%$originalTicketNumber%' and ticket_type in(1,7)  and pnr_id in (select id from ident where  agent = '$agent' and crs_id = '14'))" ;
        ...
        if (isset($ticketsFound[0]) && false == $ticketsFound[0]->exists || (static::$output == true)) {
            ...
            if ($ticketDataCollection->count() != 0 && Ticketdata::checkIssueTicketExist($ticketDataCollection, $agent,'14') == true) {
