...
//                $identCollection->put('total_pnr_passengers', matchTotalPassengerInPNR($singleLine));
        ...
        if(isPricesInMx($data[0])) {
            if (isset($fileDataArray['MX'])) {
                ...
                $priceCollection->put('tax_amount', Sab_matchTaxAmount($fileDataArray['MX']));
                $priceCollection->put('tax_currency', Sab_matchTaxCurrency($fileDataArray['MX']));
                $priceCollection->put('equiv_amount', Sab_matchEquivalentAmount($fileDataArray['MX']));
                $priceCollection->put('equiv_currency', Sab_matchEquivalentCurrency($fileDataArray['MX']));
                $priceCollection->put('emd_flag', false);

            }
        }else{
            foreach ($fileDataArray['M5'] as $single) {
                $checkReissue = trim(substr($single,7,1));
                if(strpos($single,'/E-@') === false) continue;
                //if($checkReissue == '') continue;
                
                if($checkReissue == 'A' || $checkReissue == 'E' || $checkReissue == ''){
                    //$participantsId = trim(substr($single,4,2));
                    ...
                    $priceCollection->put('equiv_currency', '');
                    $priceCollection->put('emd_flag', false);
                }else{
                    throw new Exception("Check M5 line ".__FILE__.' '.__LINE__);
                }
            }
...
//                $segmentsCollection->putByIndex('stop_over', Sab_matchStopOver($singleLine), $segmentCounter);
                ...
                $segmentsCollection->putByIndex('tax_currency', $priceCollection->offsetGet('tax_currency'), $segmentCounter);
        ...
        if(isset($fileDataArray['M5'])){
            foreach ($fileDataArray['M5'] as $singleLine){
               ...
               // Sab_matchCommissionAmount($singleLine);
                ...
                $ticketDataCollection->putByIndex('name', $participantsCollection->offsetGetByIndex('name', 0), $this->ticketCounter);
                ...
                $ticketDataCollection->putByIndex('tax_currency',$priceCollection->offsetGet('tax_currency'),$this->ticketCounter);
