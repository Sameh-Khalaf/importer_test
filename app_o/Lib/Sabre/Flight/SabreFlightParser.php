...
class SabreFlightParser
{
    ...
    public function parse($file, $agent, $output = false)
    {
        ...
        $ticketType = Sab_matchTicketType($data[0]);
        ...
        if ($ticketType == '1') {
            ...
            $this->_prepData($data, 'M1', Sab_matchM1Count($data[0]));
            $this->_prepData($data, 'M2', Sab_matchM2Count($data[0]));
            $this->_prepData($data, 'M3', Sab_matchM3Count($data[0]));
            $this->_prepData($data, 'M4', Sab_matchM4Count($data[0]));
            $this->_prepData($data, 'M5', Sab_matchM5Count($data[0]));
            $this->_prepData($data, 'M8');
;
            $this->_prepMXRecords($data);
        ...
        if($ticketType == 'A'){
            ...
            $this->_prepData($data, 'M4', Sab_matchM4Count($data[0]));
            $this->_prepData($data, 'M5', Sab_matchM5Count($data[0]));
            $this->_prepData($data, 'MG', Sab_matchM5Count($data[0]));
        ...
        switch ($ticketType) {
            case "1": //issue
                ...
                $parser->parse($file, $agent, $identCollection, $ticketDataCollection, $segmentsCollection,
                    $priceCollection, $participantsCollection, $invoiceRemarksCollection, $customRemarksCollection, $data,$this->fileDataArray);
